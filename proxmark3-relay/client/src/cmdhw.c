...
static int CmdSpeedTest(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hw speed_test", "Speed test", "");
    ...
    for(uint32_t cicle = 2; cicle <= 1300; cicle+=32){
        len = cicle;
        num_bytes += len;
        for (uint16_t i = 0; i < len; i++)
            data[i] = i & 0xFF & cicle;
        ...
        if (WaitForResponseTimeout(CMD_PING, &resp, 1000)) {
            bool error = (memcmp(data, resp.data.asBytes, len) != 0);
            PrintAndLogEx((error) ? ERR : SUCCESS, " " _GREEN_("received") " %s  len: %d   time: %d ms", error ? _RED_("NOT ok") : _GREEN_("OK"), len, msclock() - t1);
            t1 = msclock();
        } else{
            PrintAndLogEx(WARNING, "Ping response " _RED_("timeout"));
            break;
    ...
    PrintAndLogEx(NORMAL, "Num bytes %d  Time: %d\n", num_bytes, msclock() - t);
...
static command_t CommandTable[] = {
    ...
    {"speed_test",    CmdSpeedTest,    IfPm3Present,    "Speed Test."},
