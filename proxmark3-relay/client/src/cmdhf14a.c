...
// ----------------------------------------------------------------
static char* hex_text2(uint8_t* buf, int len){
...
int CmdHF14ASniffOnline(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 14a sniffo",
                  "Collect data from the field and send it to client.\n",
                  " hf 14a sniffo -c -r");
    ...
    void *argtable[] = {
        ...
        arg_lit0("c", "card", "triggered by first data from card"),
        arg_lit0("r", "reader", "triggered by first 7-bit request from reader (REQ,WUP,...)"),
        arg_lit0("p", "pipe", "send it to pipe(fifo)"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    // arg_get_lit(ctx, 3)  ??? to povzroča sagmentation fault !!!!!! (if else{....printf ....})
    if(arg_get_lit(ctx, 3)){
        if(!OpenPipe()){
            PrintAndLogExPipe(INFO, "PIPE");
            PrintAndLogExPipe(FAILED, "Failed OpenPipe()");
            fflush(stdout);
            return PM3_EFILE;
        }
        PrintAndLogExPipe(INFO, "PIPE OPEND.");
    }
    

    uart_reconfigure_timeouts(0);
    
    if (arg_get_lit(ctx, 3)) { // pipe
        SendTxtToPipe("--START SNIFF--");
        pipe = true;
    }
    ...
    clearCommandBuffer();
    SendCommandNG(CMD_HF_ISO14443A_SNIFF_ONLINE, (uint8_t *)&param, sizeof(uint8_t));
    ...
    PrintAndLogExPipe(INFO, "Ready");
    fflush(stdout);
    while (1)
    {
        WaitForResponseTimeoutW(CMD_HF_ISO14443A_SNIFF_ONLINE, &resp, -1, false);

        if(resp.length == 0){
            PrintAndLogExPipe(INFO, "--DONE--");
            break;
        }
        if(resp.length < 13){
            PrintAndLogExPipe(ERR, "length < 13 ... %d", resp.length);
            break;
        }
        recv = resp.data.asBytes;
        // print_hex(recv, resp.length);

        len = recv[0] | (recv[1] << 8);
        len -= 10;
        if(resp.length - 13 - len != 0){
            PrintAndLogExPipe(INFO, "-DONE-");
            break;
        }

        resp.data.asBytes[11] = check_crc(CRC_14443_A, resp.data.asBytes+13, len); // if crc is ok

        if (pipe) {
            SendDataToPipe(P_SNIFF, resp.length, resp.data.asBytes);
            // SendToPipe(resp.data.asBytes, resp.length);
            // pipe
        }else{ // Print
            startTime = recv[3] | (recv[4] << 8) | (recv[5] << 16) | (recv[6] << 24);
            endTime = recv[7] | (recv[8] << 8) | (recv[9] << 16) | (recv[10] << 24);
            // parity = recv[11];
            tag = recv[12];

            PrintAndLogExPipe(INFO, "%s %-8d %-3d|  %s", tag?"T:":"D:", endTime - startTime, len, hex_text2(recv+13, len));
            // for(int i = 0; i < len; i++){
            //     printf("%c%c ", HexLookUp[recv[13 + i] >> 4], HexLookUp[recv[13 + i] & 0x0F]);
            // }
            //     
            // printf("// %d\n", len);
            // fflush(stdout);
            //PrintAndLogEx(INFO, "HI");
        }
    }
    PrintAndLogExPipe(INFO, "--END--");
    fflush(stdout);
    if (pipe) {
        SendTxtToPipe("--END SNIFF--");
        ClosePipe();
    }
    return PM3_SUCCESS;
}
...
static command_t CommandTable[] = {
    ...
    {"sniffo",      CmdHF14ASniffOnline,  IfPm3Iso14443a,  "sniff ISO 14443-a traffic Online"},
    {"relay",       CmdHF14ARelayProxy,   IfPm3Iso14443a,  "relay proxy"},
    {"relay_mole",  CmdHF14ARelayMole,    IfPm3Iso14443a,  "relay mole"},
...
int CmdHF14ARelayMole(const char *Cmd){
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 14a relay_mole",
                  ".......\n",
                  " hf 14a relay ...... -c -r");
...
int CmdHF14ARelayProxy(const char *Cmd){
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 14a relay_proxy",
                  ".......\n",
                  " hf 14a relay ...... -c -r");
