...
void PrintAndLogExPipe(logLevel_t level, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    char buffer[2048] = {0};
    int len = vsnprintf(buffer, sizeof(buffer), fmt, args);
    va_end(args);
    PrintAndLogEx(level, "%s", buffer);
    if(IsPipeOpen()){
        uint8_t * data = (uint8_t*)calloc(len+5, sizeof(uint8_t));
        data[0] = ((len+2) >> 0) & 0xFF;
        data[1] = ((len+2) >> 8) & 0xFF;
        data[2] = 8; // LOG
        data[3] = 1; // MOLE this will be change on main mole side.
        data[4] = 2;
        if(level == WARNING){
            data[4] = 3;
        }else if(level == ERR){
